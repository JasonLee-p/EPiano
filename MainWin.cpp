...
void MainWin::initInputConfig() {
	// 初始化默认音量，绑定release信号与DEFAULT_VELOCITY的修改
	Key::DEFAULT_VELOCITY = ui.velocitySlider->value();
	connect(ui.velocitySlider, &QSlider::sliderReleased, this, [this]() {
		mutex.lock();
		Key::DEFAULT_VELOCITY = ui.velocitySlider->value();
		mutex.unlock();
	});
	...
	// 绑定slider的mouseRelease信号与ENVELOPE_PARAMETERS的修改，以及全体SynthNote的envelope的修改
#define BIND_SLIDER_RELEASED(slider, index) \
	connect(slider, &QSlider::sliderReleased, this, [this]() { \
		mutex.lock(); \
		LoadConfig::ENVELOPE_PARAMETERS[index] = slider->value(); \
		for (int i = 0; i < 88; i++) { \
			delete SynthNote::noteMap[i]->envelope; \
			SynthNote::noteMap[i]->envelope = new Envelope(); \
		} \
		mutex.unlock(); \
	});
