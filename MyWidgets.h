...
protected:
    void mousePressEvent(QMouseEvent* event) override
    {
		...
		// 在鼠标右侧显示当前值
		QPoint glb_pos_ = mapToGlobal(QPoint(0, 0));
		QToolTip::showText(glb_pos_, QString::number(value()), this);
	}
	...
	void wheelEvent(QWheelEvent* event) override {
		if (event->angleDelta().y() > 0 && value() + singleStep() <= maximum()) {
			setValue(value() + singleStep());
		}
		else if (event->angleDelta().y() < 0 && value() - singleStep() >= minimum()) {
			setValue(value() - singleStep());
		}
		// 在鼠标右侧显示当前值
		QPoint glb_pos_ = mapToGlobal(QPoint(0, 0));
